...
describe('Test for person', () => {
    ...
    describe('test for calcBMI', () => {
        it('should return a string: down', () => {
            person.weight = 40;
            expect(person.calcBMI()).toEqual('down');
        });

        it('should return a string: normal', () => {
            person.weight = 58;
            expect(person.calcBMI()).toEqual('normal');
        });

        it('should return a string: overweight', () => {
            person.weight = 68;
            expect(person.calcBMI()).toEqual('overweight');
        });

        it('should return a string: overweight level 1', () => {
            person.weight = 75;
            expect(person.calcBMI()).toEqual('overweight level 1');
        });

        it('should return a string: overweight level 2', () => {
            person.weight = 90;
            expect(person.calcBMI()).toEqual('overweight level 2');
        });

        it('should return a string: overweight level 3', () => {
            person.weight = 120;
            expect(person.calcBMI()).toEqual('overweight level 3');
        });

        it('should return a string: no found', () => {
            person.weight = -48;
            expect(person.calcBMI()).toEqual('no found');
            person.weight = -48;
            person.height = -1.70;
            expect(person.calcBMI()).toEqual('no found');
        })
